...
const contractStore = {
  ...
  actions: {
    ...
    createContractVehicle({ commit, getters }, { cVehicleData }) {
      ...
      let formData = {
        'contract_vehicle[name]': cVehicleData.name,
        'contract_vehicle[full_name]': cVehicleData.fullName,
        'contract_vehicle[contract_sub_category_id]': cVehicleData.subCatId,
        'contract_vehicle[contract_agency_id]': cVehicleData.cAgencyId,
        'contract_vehicle[contract_vehicle_type_id]': cVehicleData.type,
        'contract_vehicle[contract_number_id]': cVehicleData.contract_number_id,
        'contract_vehicle[ceiling]': cVehicleData.ceiling
      }
      if (cVehicleData.cafFees) {
        formData['contract_vehicle[caf_fees]'] = cVehicleData.cafFees
      }
      if (cVehicleData.subprime_name) {
        formData['contract_vehicle[subprime_name]'] = cVehicleData.subprime_name
      }
      // prime
      if (cVehicleData.prime_name) {
        formData['contract_vehicle[prime_name]'] = cVehicleData.prime_name
      }
      // contract_name
      if (cVehicleData.contract_name) {
        formData['contract_vehicle[contract_name]'] = cVehicleData.contract_name
      }
      formData['contract_vehicle[base_period_start]'] = cVehicleData.bp_startDate
      formData['contract_vehicle[base_period_end]'] = cVehicleData.bp_endDate
      formData['contract_vehicle[option_period_start]'] = cVehicleData.op_startDate
      formData['contract_vehicle[option_period_end]'] = cVehicleData.op_endDate
      axios({
        method: 'POST',
    ...
    updateContractVehicle({ commit, getters }, { cVehicleData, id }) {
      ...
      if (cVehicleData.co_poc_ids && cVehicleData.co_poc_ids.length > 0) {
        cVehicleData.co_poc_ids.forEach((ids) => {
          formData['contract_vehicle[co_contract_poc_ids][]'] = ids
          // formData.append('contract_vehicle[co_contract_poc_ids][]', ids)
        })
      } else {
        formData['contract_vehicle[co_contract_poc_ids][]'] = []
      ...
      if (cVehicleData.gov_poc_ids && cVehicleData.gov_poc_ids.length > 0) {
        cVehicleData.gov_poc_ids.forEach((ids) => {
          formData['contract_vehicle[gov_contract_poc_ids][]'] = ids
          // formData.append('contract_vehicle[gov_contract_poc_ids][]', ids)
        })
      } else {
        formData['contract_vehicle[gov_contract_poc_ids][]'] = []
      ...
      if (cVehicleData.pm_poc_ids && cVehicleData.pm_poc_ids.length > 0) {
        cVehicleData.pm_poc_ids.forEach((ids) => {
          formData['contract_vehicle[pm_contract_poc_ids][]'] = ids
          // formData.append('contract_vehicle[pm_contract_poc_ids][]', ids)
        })
      } else {
        formData['contract_vehicle[pm_contract_poc_ids][]'] = []
  ...
  mutations: {
    ...
    SET_ASSOCIATED_CONTRACTS_STATUS: (state, status) =>
      (state.associated_contracts_status = status),
    TOGGLE_ASSOCIATED_CONTRACTS_LOADED: (state, loaded) =>
      (state.associated_contracts_loaded = loaded),
    ...
    TOGGLE_ASSOCIATED_VEHICLES_LOADED: (state, loaded) =>
      (state.associated_vehicles_loaded = loaded),
