...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setShowProjectStats: (state, showStats) => (state.showProjectStats = showStats),
    setShowContractStats: (state, showStats) => (state.showContractStats = showStats),
    setShowVehicleStats: (state, showStats) => (state.showVehicleStats = showStats),
    ...
    setContactInfoForm: (state, { key, value }) => (state.contactInfoForm[key] = value),
    setMapZoomFilter: (state, filteredIds) => (state.mapZoomFilter = filteredIds),
  ...
  getters: {
    ...
    filterDataForAdvancedFilter:
      (state, getters) =>
      (resources, page_name, facility = null) => {
          ...
          _.each(state.mapFilters, (f) => {
            ...
            switch (k) {
              case 'advancedFilter': {
                valid =
                  valid &&
                  getters.filterDataForAdvancedFilter(resources1, 'filteredFacilities', facility)
                break
              }
              case 'dueDate': {
                let range = moment.range(f[k][0], f[k][1])
                valid =
                  valid && facility[k] && range.contains(new Date(facility[k].replace(/-/g, '/')))
                break
              }
              case 'noteDate': {
        ...
        _.each(state.mapFilters, (f) => {
          ...
          switch (k) {
            case 'advancedFilter': {
              valid =
                valid &&
                getters.filterDataForAdvancedFilter(resources1, 'filteredContracts', contract)
              break
            }
            case 'dueDate': {
              let range = moment.range(f[k][0], f[k][1])
              valid =
                valid && contract[k] && range.contains(new Date(contract[k].replace(/-/g, '/')))
              break
            }
            case 'noteDate': {
              let taksIssues = contract.tasks.concat(contract.issues).concat(contract.risks)
              let resources = []
              for (let r of taksIssues) {
                var v = getters.filterDataForAdvancedFilter([r], 'filteredContracts', contract)
                if (v) {
                  resources.push(r)
                }
              }
            ...
            // TODO: Improve this function
            case 'taskIssueProgress': {
              let progressFor = contract.tasks.concat(contract.issues).concat(contract.risks)
              let resources = []
              for (let r of progressFor) {
                var v = getters.filterDataForAdvancedFilter([r], 'filteredContracts', contract)
                if (v) {
            ...
            case 'taskTypeIds': {
              var tasksIssues = contract.tasks.concat(contract.issues).concat(contract.risks)
              var resources = _.filter(tasksIssues, (ti) => f[k].includes(ti.taskTypeId))
              if (resources.length < 1) {
                valid = false
              }
            ...
            case 'taskIssueUsers': {
              var taskIssues = contract.tasks.concat(contract.issues).concat(contract.risks)
              var resources = _.filter(
                taskIssues,
                (ti) => _.intersection(ti.userIds, f[k]).length > 0
              )
        ...
        _.each(state.mapFilters, (f) => {
          ...
          switch (k) {
            case 'advancedFilter': {
              valid =
                valid &&
                getters.filterDataForAdvancedFilter(resources1, 'filteredVehicles', vehicle)
              break
            }
            case 'dueDate': {
              let range = moment.range(f[k][0], f[k][1])
              valid = valid && vehicle[k] && range.contains(new Date(vehicle[k].replace(/-/g, '/')))
              break
            }
            case 'noteDate': {
              let taksIssues = vehicle.tasks.concat(vehicle.issues).concat(vehicle.risks)
              let resources = []
              for (let r of taksIssues) {
                var v = getters.filterDataForAdvancedFilter([r], 'filteredVehicles', vehicle)
                if (v) {
                  resources.push(r)
                }
              }
            ...
            // TODO: Improve this function
            case 'taskIssueProgress': {
              let progressFor = vehicle.tasks.concat(vehicle.issues).concat(vehicle.risks)
              let resources = []
              for (let r of progressFor) {
                var v = getters.filterDataForAdvancedFilter([r], 'filteredVehicles', vehicle)
                if (v) {
            ...
            case 'taskTypeIds': {
              var tasksIssues = vehicle.tasks.concat(vehicle.issues).concat(vehicle.risks)
              var resources = _.filter(tasksIssues, (ti) => f[k].includes(ti.taskTypeId))
              if (resources.length < 1) {
                valid = false
              }
            ...
            case 'taskIssueUsers': {
              var taskIssues = vehicle.tasks.concat(vehicle.issues).concat(vehicle.risks)
              var resources = _.filter(
                taskIssues,
                (ti) => _.intersection(ti.userIds, f[k]).length > 0
              )
    ...
    facilityGroupFacilities:
      ...
      (group, status = 'active') => {
        let h = {
          projects: {
            a: getters
              .filteredFacilities(status)
              .filter(
                (f) => f.facilityGroupId == group.id && f.projectId == getters.currentProject.id
              )
              .sort((a, b) => a.facilityName.localeCompare(b.facilityName))
          },
          contracts: {
            b: getters.filteredContracts
              .filter((f) => f.facilityGroup.id == group.id)
          ...
          if (f_read) {
            ...
            for (let type in types) {
              ...
              let ranges = getters.taskProgressFilter
                ? _.map(getters.taskProgressFilter, 'value').map((r) => r.split('-').map(Number))
                : false
              let stages =
    ...
    filteredAllContractTasks: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredContracts, 'tasks')), (t) => {
        let valid = true
        if (ids.length > 0) valid = valid && ids.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.taskStageId)
    ...
    filteredAllVehicleTasks: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredVehicles, 'tasks')), (t) => {
        let valid = true
        if (ids.length > 0) valid = valid && ids.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.taskStageId)
    ...
    filteredAllContractIssues: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredContracts, 'issues')), (t) => {
        let valid = true
        if (ids.length > 0) valid = valid && ids.includes(t.issueTypeId)
        if (taskTypeIds.length > 0) valid = valid && taskTypeIds.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.issueStageId)
    ...
    filteredAllVehicleIssues: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredVehicles, 'issues')), (t) => {
        let valid = true
        if (ids.length > 0) valid = valid && ids.includes(t.issueTypeId)
        if (taskTypeIds.length > 0) valid = valid && taskTypeIds.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.issueStageId)
    ...
    filteredAllRisks: (state, getters) => {
      ...
      let riskPriorityLevels =
        getters.riskPriorityLevelFilter && getters.riskPriorityLevelFilter.length
      ...
      return _.filter(_.flatten(_.map(getters.filteredFacilities('active'), 'risks')), (t) => {
        ...
        if (riskPriorityLevels.length > 0)
          valid = valid && riskPriorityLevels.includes(t.riskPriorityLevelIds)
    ...
    filteredAllContractRisks: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredContracts, 'risks')), (t) => {
        let valid = true
        if (approaches.length > 0) valid = valid && approaches.includes(t.riskApproachFilterIds)
        if (taskTypeIds.length > 0) valid = valid && taskTypeIds.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.riskStageId)
    ...
    filteredAllVehicleRisks: (state, getters) => {
      ...
      return _.filter(_.flatten(_.map(getters.filteredVehicles, 'risks')), (t) => {
        let valid = true
        if (approaches.length > 0) valid = valid && approaches.includes(t.riskApproachFilterIds)
        if (taskTypeIds.length > 0) valid = valid && taskTypeIds.includes(t.taskTypeId)
        if (stages.length > 0) valid = valid && stages.includes(t.riskStageId)
  ...
  actions: {
    ...
    verifyToken({ commit, getters }, payload) {
      http
        .get(`${API_BASE_PATH}/auth/verify_token?token=${getters.getToken}`)
        .then((res) => {
    ...
    fetchCurrentProject({ commit, dispatch }, id) {
      let spaths = window.location.hash.split('/')
    ...
    fetchFacility({ commit, dispatch, getters }, { projectId, facilityId }) {
      return new Promise((resolve, reject) => {
        http
          .get(`${API_BASE_PATH}/programs/${projectId}/projects/${facilityId}.json`)
          .then((res) => {
    ...
    taskDeleted({ commit }, { task, programId }) {
      ...
      return new Promise((resolve, reject) => {
        http
          .delete(`${API_BASE_PATH}/${deleteUrl}`)
          .then((res) => {
            if (task.facilityId) {
