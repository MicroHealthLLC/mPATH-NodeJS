      ...
      <div>
        <h5 class="mb-0">
          <span style="font-size: 16px; margin-right: 10px"
            ><i class="far fa-car text-info"></i></span>
           <router-link :to="backToVehicle">
             ...
             <span v-else>{{
                   vehicleLesson.vehicle_nickname
                  }}
              </span>
            </router-link>     
          <el-icon
          ...
          <el-icon
            ...
            style="font-size: 12px"
          ></el-icon>
          <span v-if="vehicleLesson.title">{{ vehicleLesson.title }}</span>
    ...
    <div v-show="currentTab == 'tab1'" class="row mt-2 mx-0">
      <div class="col-12 px-0">
        ...
        >
          <span
            ...
            v-tooltip="`Draft`"
          >
            <span  v-show="vehicleLesson.draft">
          ...
          <span
            ...
            v-tooltip="`Important`"
          >
            <span v-show="vehicleLesson.important">
          ...
          <span
            ...
            v-tooltip="`Briefings`" 
          >
            <span v-show="vehicleLesson.reportable">
      ...
      <div class="col-12 px-0">
        ...
        <el-input
          ...
          type="textarea"
          v-validate="'required'"
          v-model="vehicleLesson.description"
      ...
      <div class="col-12 p-0">
        <div class="d-flex justify-content-between my-3">
          ...
          ><button
            v-show="vehicleLesson.lesson_stage_id"
        ...
        <el-steps
          :active="
            lessonStages.findIndex(
              (stage) => stage.id == vehicleLesson.lesson_stage_id
            )
          "
          finish-status="success"
          v-model="vehicleLesson.lesson_stage_id"
          value-key="id"
          track-by="id"
...
export default {
  ...
  methods: {
    ...mapActions(["fetchVehicleLesson", "updateVehicleLesson", "addVehicleLesson"]),
    ...mapMutations(["SET_VEHICLE_LESSON", "SET_VEHICLE_LESSON_STATUS"]),
    saveLesson() {
      this.$validator.validate().then((success) => {
        ...
        let lessonData = {
          lesson: {
            title: this.vehicleLesson.title,
            description: this.vehicleLesson.description,
            date: this.vehicleLesson.date,
            task_type_id: this.vehicleLesson.task_type_id,
            lesson_stage_id: this.vehicleLesson.lesson_stage_id,
            important: this.vehicleLesson.important,
            reportable: this.vehicleLesson.reportable,
            draft: this.vehicleLesson.draft,
            // Array values below
   ...
   // Check to add or update existing lesson by confirming an id
    ...
    removeEmptySFBP(sFBP){
      var returnSFBP = [];
      for (let i in sFBP) {
        if(!sFBP[i].finding && !sFBP[i]._destroy && !(this.vehicleLesson.draft && sFBP[i].recommendation)) continue;
        returnSFBP.push(sFBP[i]);        
      }
      return [...sFBP];
    },
    _isallowed(salut) {
      return this.checkPrivileges("VehicleLessonForm", salut, this.$route)
     },
    close() {
    ...
    changeStage(stage) {
      if (this.vehicleLesson.id && this._isallowed("write")) {
        this.vehicleLesson.lesson_stage_id = stage.id;
      } else if (this._isallowed("write")) {
        this.SET_LESSON({ ...this.lesson, lesson_stage_id: stage.id });
      }
    },
    clearStage() {
      this.vehicleLesson.lesson_stage_id = null;
    },
    toggleImportant() {
      if(!this._isallowed('write')){
        return
      }
      this.SET_VEHICLE_LESSON({ ...this.vehicleLesson, important: !this.vehicleLesson.important });
    },
    toggleDraft() {
      if(!this._isallowed('write')){
        return
      }
      this.SET_VEHICLE_LESSON({ ...this.vehicleLesson, draft: !this.vehicleLesson.draft });
    },
    toggleReportable() {
      if(!this._isallowed('write')){
        return
      }
      this.SET_VEHICLE_LESSON({ ...this.vehicleLesson, reportable: !this.vehicleLesson.reportable });
    },
    log(e)
    {
      // console.log(""+e);
 ...
 isProgramView() {
  return this.$route.name.includes("ProgramTaskForm") ||
          ...
          this.$route.name.includes("ProgramVehicleLessonForm") ||
          this.$route.name.includes("ProgramLessonForm") ;
    },
  backToLessons() {
    if (this.$route.params.vehicleId && !this.isProgramView) {
        return  `/programs/${this.$route.params.programId}/${this.tab}/vehicles/${this.$route.params.vehicleId}/lessons`
      } else {
   ...
   backToVehicle(){
    if (this.isProgramView){
      return `/programs/${this.$route.params.programId}/sheet/vehicles/${this.$route.params.vehicleId}/` 
    } else return `/programs/${this.$route.params.programId}/${this.tab}/vehicles/${this.$route.params.vehicleId}/`;
     },
    isMapView() {
    ...
    vehicleLesson: {
      handler(newValue, oldValue) {
        if (
          ...
          this.relatedTasks = this.vehicleLesson.sub_tasks;
          this.relatedIssues = this.vehicleLesson.sub_issues;
          this.important = this.vehicleLesson.important;
          this.draft = this.vehicleLesson.draft;
          this.reportable = this.vehicleLesson.reportable;
          this.relatedRisks = this.vehicleLesson.sub_risks;
          this.successes = this.vehicleLesson.successes;
          ...
          this.files = this.vehicleLesson.attach_files.filter((file) => !file.link);
          this.fileLinks = this.vehicleLesson.attach_files.filter((file) => file.link);
        }
      },
    },
    contentLoaded: {
      handler() {
        if (this.vehicleLesson) {
          this.relatedTasks = this.vehicleLesson.sub_tasks;
          this.relatedIssues = this.vehicleLesson.sub_issues;
          this.important = this.vehicleLesson.important;
          this.draft = this.vehicleLesson.draft;
