    ...
    <div v-if="_isallowed('read')">
      ...
      <span :load="log(fridayDayOfWeek)">
        <div class="form-group w-100 mr-1 row">
          ...
          <div class="col-3">
            ...
            <el-input type="search" placeholder="Filter by Task" aria-label="Search" aria-describedby="search-addon"
              v-model="tasksQuery" data-cy="search_tasks">
          ...
          <div class="col-2 mt-1 pr-0">
            ...
            <el-select v-model="weekBegin" class="mr-1 d-flex" track-by="id" value-key="id"
              placeholder="Select Week of Begin" filterable>
              <el-option v-for="item, i in weekOfBegin" :value="item" :key="i" :label="item">
              </el-option>
            </el-select>
          </div>
          ...
          <div class="col-2 mt-1 pl-0">
            ...
            <el-select v-model="weekEnd" class="d-flex" track-by="id" value-key="id" placeholder="Select Week of End"
              filterable>
              <el-option v-for="item, i in weekOfEnd" :value="item" :key="i" :label="item">
              </el-option>
            </el-select>
          </div>
          ...
          <div class="col-1 mt-4 px-0">
            <el-switch v-model="taskProgressFilter" active-text="Active Tasks" inactive-text="All Tasks*">
            </el-switch>
            <div v-show="false">
      ...
      <el-tabs type="border-card" v-model="editableTabsValue" v-if="$route.params.projectId">
        <el-tab-pane label="Summary" class="is-active" name='0'>
          ...
          <div v-else>
            You currently have no Tasks or *Excluded Tasks in this Project. Please add a task or adjust the <em>Week of
              End </em> filter.
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane v-for="(item) in efforts" :key="item.id" :label="item.full_name">
          ...
          <el-table v-if="tableData && tableData.length > 0 && matrixDates && matrixDates.length > 0" :data="tableData"
            height="450" id="crudRow" class="crudRow mt-4" :header-row-style="{ textAlign: 'center' }">
            <el-table-column prop="plannedEffort" label="Planned Effort" width="80" header-align="center">
            </el-table-column>
            <el-table-column prop="actualEffort" label="Actual Effort" width="80" header-align="center">
                ...
                <template slot-scope="scope">
                  ...
                  <span v-if="rowId == scope.row.id">
                    <span v-if="item.tasks && item.tasks.length > 0 && getWeekOfEffort(item.tasks, scope.row, weekof)">
                      <input @change="changeValues($event, weekof, item, rowId,
                        item.tasks
                          .filter(t => t.id == scope.row.id)
                          .map(t => t.efforts)
                          .flat()
                          .filter(t => t.date_of_week == weekof)
                      )" :value="getWeekOfEffort(item.tasks, scope.row, weekof, item)" :name="weekof" type="text"
                        class="form-control" />
                      <!-- CREATE MODE IF ENTER/EDIT BUTTON IS CLICKED -->
                    </span>

                    <input v-else v-model="addedHrs[weekofIndex]" :name="weekof" type="text" class="form-control"
                      :id="weekof" />
                  </span>
                </template>
              </el-table-column>
            ...
            <el-table-column  v-if="_isallowed('write')" label="Actions" width="110" align="center">
              <template slot-scope="scope">
                ...
                <el-button size="mini" type="default" v-tooltip="`Add/Edit Effort`" class="bg-light px-2"
                  v-if="scope.$index !== rowIndex" @click.prevent="editToggle(scope.$index, scope.row, rowIndex, item)">
                  <i class="fa-light fa-calendar-pen text-primary"></i>
                </el-button>
                <el-button type="default" size="mini"
                  @click.prevent="saveEffortRow(scope.$index, scope.row, item.id, item.tasks, weekof)" v-tooltip="`Save`"
                  class="bg-primary text-light  px-2" :disabled="rowId && rowId !== scope.row.id">
          ...
          <div v-else>
            You currently have no Tasks in this Project or the Task due date has passed. Please add a task or adjust the
            <em>Week of End</em> filter.
          </div>
...
export default {
  ...
  methods: {
    ...
    _isallowed(salut) {
      return this.checkPrivileges("effort_sheets", salut, this.$route)
    },
    log(e) {
