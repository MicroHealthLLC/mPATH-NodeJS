  ...
  <div class="container-fluid">
    <div style="height:85px" class="pt-2">
      <span @click.prevent="backHomeBtn">
        <img style="width: 147px; cursor:pointer" :src="srcLogo" />
        <!-- <h3 class="d-inline mt-1 programName">{{ this.$portfolio_heading }}</h3> -->
      </span>
      <span class="float-right mr-4">
        <button class="portfolioHomeBtn mh-orange btn" style="cursor: pointer" @click.prevent="backHomeBtn">
    ...
    <el-tabs :tab-position="tabPosition" type="border-card" class="bottomTabs" @tab-click="handleClick">
      <el-tab-pane label="" v-loading="!contractVehiclesLoaded"
      ...
      <el-tab-pane v-loading="!contractProjectsLoaded" element-loading-text="Fetching your data. Please wait..."
        element-loading-spinner="el-icon-loading">
        <span slot="label"> <i class="far fa-file-contract mr-1"
            :class="[pane1 ? 'mh-orange-text' : 'txt-secondary']"></i>CONTRACT DETAILS</span>
        <div style="height:80vh" class="portfolio-contracts-module">
          <div style="height: 80vh; overflow-y:auto">
            <el-table :data="tableData" border height="800" style="width: 95%" :summary-method="getSummaries"
              show-summary>
              <el-table-column fixed label="Code*" width="" prop="charge_code">
                <template slot-scope="scope">
                  <el-input size="small" tabindex="1" v-if="_isallowed('write') && scope.$index == createRow"
                    class="p-0" style="text-align:center" v-model="scope.row.charge_code"
                    controls-position="right"></el-input>
                  <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                    <el-input size="small" style="text-align:center" class="p-0" v-model="scope.row.charge_code"
                      controls-position="right"></el-input>
                  </span>
              ...
              <el-table-column fixed label="Project Name*" width="200">
                <template slot-scope="scope">
                  <el-input size="small" tabindex="2" v-if="_isallowed('write') && scope.$index == createRow"
                    style="text-align:center" v-model="scope.row.name" controls-position="right"></el-input>
                  <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                    <el-input size="small" style="text-align:center" v-model="scope.row.name"
                      controls-position="right"></el-input>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                    {{ scope.row.name }}
              ...
              <el-table-column label="Customer*" width="200">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="3" v-model="scope.row.contract_customer_id" filterable track-by="name"
                      value-key="id" class="w-100" placeholder="" clearable allow-create default-first-option
                      size="small">
        ...
        && (scope.row.contract_customer && scope.row.contract_customer.name !== null)">
              ...
              <el-table-column label="Vehicle/ Schedule" width="125">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (scope.$index == createRow)">
                    <el-select v-model="scope.row.contract_vehicle_id" filterable tabindex="4" track-by="name"
                      value-key="id" class="w-100" clearable allow-create placeholder="" default-first-option
                      size="small">
                      <el-option v-for="item in vehicleOptions" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="_isallowed('write') && (rowId == scope.row.id && scope.$index !== createRow)">
                    <span v-if="scope.row.contract_vehicle && scope.row.contract_vehicle_id">
                      <el-select v-model="scope.row.contract_vehicle_id" filterable track-by="name" value-key="id"
                        class="w-100" clearable allow-create placeholder="" default-first-option size="small">
                        <el-option v-for="item in vehicleOptions" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                      </el-select>
                    </span>
                    ...
                    <span v-else>
                      <el-select v-model="blankVehicle" filterable track-by="name" value-key="id" class="w-100"
                        clearable allow-create placeholder="" default-first-option size="small">
                        <el-option v-for="item in vehicleOptions" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                      </el-select>
                    </span>
        ...
        (scope.row.contract_vehicle && scope.row.contract_vehicle.name !== null)">
              ...
              <el-table-column label="Contract Number**" width="125">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="5" v-model="scope.row.contract_number_id" filterable track-by="name"
                      value-key="id" class="w-100" placeholder="" clearable allow-create default-first-option
                      size="small">
                      <el-option v-for="item in contractNumbers" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_number && scope.row.contract_number.name !== null)">
              ...
              <el-table-column label="Award/ TO Number**" width="125">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="6" v-model="scope.row.contract_award_to_id" filterable track-by="id"
                      placeholder="" value-key="id" class="w-100" clearable allow-create default-first-option
                      size="small">
                      <el-option v-for="item in awardToNums" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_award_to && scope.row.contract_award_to.name !== null)">
              ...
              <el-table-column label="NAICS*" width="70">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="7" v-model="scope.row.contract_naic_id" filterable track-by="name"
                      value-key="id" class="w-100" placeholder="" clearable allow-create default-first-option
                      size="small">
                      <el-option v-for="item in naicsOptions" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_naic && scope.row.contract_naic.name !== null)">
              ...
              <el-table-column label="Award Type*" width="70">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="8" v-model="scope.row.contract_award_type_id" filterable track-by="name"
                      value-key="id" class="w-100" clearable placeholder="" allow-create default-first-option
                      size="small">
                      <el-option v-for="item in awardTypes" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_award_type && scope.row.contract_award_type.name !== null)">
              ...
              <el-table-column label="Contract Type*" width="75">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="9" v-model="scope.row.contract_type_id" filterable track-by="name"
                      placeholder="" value-key="id" class="w-100" clearable allow-create default-first-option
                      size="small">
        ...
        (scope.row.contract_type && scope.row.contract_type.name !== null)">
              ...
              <el-table-column label="Prime vs Sub" width="55">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="10" v-model="scope.row.prime_or_sub" track-by="name" placeholder=""
                      value-key="id" class="w-100" clearable default-first-option size="small">
                      <el-option v-for="item in primeOrSub" :key="item" :label="item" :value="item">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
              ...
              <el-table-column label="Contract Start Date*" width="125" prop="contract_start_date">
                <template slot-scope="scope">
                  <v2-date-picker tabindex="11" name="Date" v-if="_isallowed('write') && scope.$index == createRow"
                    v-model="newContractStartDate" value-type="YYYY-MM-DD" format="M/DD/YYYY" class="w-100" />
                  <span v-if="_isallowed('write') && (rowId == scope.row.id && scope.$index !== createRow)">
                    <v2-date-picker name="Date" v-model="contractStartDate" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                      class="w-100" />
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
              ...
              <el-table-column label="Contract End Date*" width="125" prop="contract_end_date">
                <template slot-scope="scope">
                  <v2-date-picker tabindex="12" name="Date" v-model="newContractEndDate"
                    v-if="(scope.$index == createRow) && _isallowed('write')"
                    :disabled-date="disabledNewContractEndDate" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                    class="w-100" />
                  <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                    <v2-date-picker name="Date" :disabled-date="disabledContractEndDate" v-model="contractEndDate"
                      value-type="YYYY-MM-DD" format="M/DD/YYYY" class="w-100" />
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
              ...
              <el-table-column label="Total Contract Value*" width="175" prop="total_contract_value">
                <template slot-scope="scope">
                  <el-input tabindex="13" size="small" v-if="_isallowed('write') && scope.$index == createRow"
                    type="number" step="0.01" style="text-align:center" v-model="scope.row.total_contract_value"
                    controls-position="right"></el-input>
                  <span v-if="_isallowed('write') && (rowId == scope.row.id && scope.$index !== createRow)">
                    <el-input size="small" :step="0.01" type="number" style="text-align:center"
                      v-model="scope.row.total_contract_value" controls-position="right"></el-input>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
              ...
              <el-table-column label="PoP's*" width="100">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="14" v-model="scope.row.contract_pop_id" filterable track-by="name"
                      value-key="id" class="w-100" placeholder="" clearable allow-create default-first-option
                      size="small">
                      <el-option v-for="item in pops" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_pop && scope.row.contract_pop.name !== null)">
              ...
              <el-table-column label="Current PoP" width="100">
                <template slot-scope="scope">
                  <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                    <el-select tabindex="15" v-model="scope.row.contract_current_pop_id" filterable track-by="name"
                      value-key="id" class="w-100" placeholder="" clearable allow-create default-first-option
                      size="small">
                      <el-option v-for="item in currentPops" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
        (scope.row.contract_current_pop && scope.row.contract_current_pop.name !== null)">
              ...
              <el-table-column label="Contract PoP Start Date*" width="100" prop="contract_current_pop_start_date">
                <template slot-scope="scope">
                  <v2-date-picker tabindex="16" name="Date" v-model="newPopStartDate"
                    v-if="scope.$index == createRow && _isallowed('write')" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                    class="w-100" />
                  <span v-if="rowId == scope.row.id && scope.$index !== createRow">
                    <v2-date-picker name="Date" v-model="popStartDate" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                      class="w-100" />
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                    <span
                      v-if="scope.row.contract_current_pop_start_date == null || scope.row.contract_current_pop_start_date == undefined">
                    </span>
              ...
              <el-table-column label="Contract PoP End Date*" width="100" prop="contract_current_pop_end_date">
                <template slot-scope="scope">
                  <v2-date-picker tabindex="16" name="Date" v-model="newPopEndDate"
                    v-if="scope.$index == createRow && _isallowed('write')" :disabled-date="disabledNewPoPEndDate"
                    value-type="YYYY-MM-DD" format="M/DD/YYYY" class="w-100" />
                  <span v-if="rowId == scope.row.id && scope.$index !== createRow">
                    <v2-date-picker name="Date" :disabled-date="disabledPoPEndDate" v-model="popEndDate"
                      value-type="YYYY-MM-DD" format="M/DD/YYYY" class="w-100" />
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                    <span
                      v-if="scope.row.contract_current_pop_end_date == null || scope.row.contract_current_pop_end_date == undefined">
                    </span>
              ...
              <el-table-column label="Notes" width="350">
                <template slot-scope="scope">
                  <el-input tabindex="17" size="small" v-if="_isallowed('write') && scope.$index == createRow"
                    style="text-align:center" v-model="scope.row.notes" controls-position="right"></el-input>
                  <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                    <el-input size="small" style="text-align:center" v-model="scope.row.notes"
                      controls-position="right"></el-input>
                  </span>
                  <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                    {{ scope.row.notes }}
              ...
              <el-table-column label="Actions" width="155" v-if="_isallowed('write') || _isallowed('delete')"
                fixed="right" align="right">
                <template slot-scope="scope">
                  <el-button size="mini" type="default" @click="saveContractProject(scope.$index, scope.row)" v-if="scope.$index == rowIndex && (
        ...
        && scope.row.contract_pop_id && popStartDate && popEndDate
                  ...
                  <el-popover v-if="programNames && (scope.$index !== rowIndex) && (scope.$index !== createRow) &&
        scope.row.associated_projects && scope.row.associated_projects.length > 0" placement="left" width="auto"
                    trigger="hover">
                    <el-button size="mini" v-for="item, i in scope.row.associated_projects" :key="i"
                      @click="openContractTask(scope.$index, scope.row, programNames.filter(t => item.id == t.program_id)[0].program_id)">
                      <span v-if="programNames.filter(t => item.id == t.program_id)[0]">{{ programNames.filter(t =>
        item.id == t.program_id)[0].label }}</span>
                    </el-button>
                    <el-button size="mini" slot="reference" type="default" v-tooltip="`Open Contract Tasks`"
                      class="bg-light text-light mr-2 px-2">
                      <i class="far fa-suitcase text-secondary"></i>
                    </el-button>
                  </el-popover>
                  <el-button size="mini" type="default" v-tooltip="`Cancel Edit`" v-if="scope.$index == rowIndex"
                    @click.prevent="cancelEdits(scope.$index, scope.row)" class="bg-secondary text-light px-2">
                    <i class="fas fa-ban"></i>
                  </el-button>
                  <el-button size="mini" type="default" v-tooltip="`Remove expiration date exemption`"
                    v-if="scope.$index == rowIndex && scope.row.ignore_expired == true"
                    @click.prevent="setIgnoreStatus(scope.$index, scope.row)" class="bg-light px-2">
                    <i class="fa-solid fa-calendar-xmark text-danger"></i>
                  </el-button>
                  <el-button size="mini" type="default" v-tooltip="`Edit`" class="bg-light px-2"
                  ...
                  <el-button size="mini" type="default" v-tooltip="`Delete`" class="bg-light px-2"
                  ...
                  <el-button size="mini" type="default" @click="saveContractProject(scope.$index, scope.row)" v-if="scope.$index == createRow && (
        ...
        && checkEmpty(scope.row.contract_pop_id) && newPopStartDate && newPopEndDate
                  ...
                  <el-button size="mini" type="default" v-tooltip="`Cancel`" v-if="scope.$index == createRow && (
        ...
        || scope.row.contract_pop_id || scope.row.contract_current_pop_id || newPopStartDate || newPopEndDate || scope.row.notes
      )" @click.prevent="cancelNewRow(scope.row)" class="bg-secondary text-light px-2">
                    <i class="fas fa-ban"></i>
                  </el-button>
                </template>
            ...
            <el-dialog :visible.sync="pocDialogVisible" append-to-body center class="p-0 users">
              ...
              <div class="container">
                <div class="row">
                  <div class="col-12" v-if="pocData">
                    <el-table :data="pocData" border height="400" style="width: 95%">
                      <el-table-column fixed label="Name*" width="150" prop="name">
                        <template slot-scope="scope">
                          <span v-if="_isallowed('write') && (scope.$index == pocCreateRow)">
                            <el-input tabindex="1" size="small" style="text-align:center" placeholder=""
                              v-model="scope.row.name" controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" style="text-align:center" placeholder="" v-model="scope.row.name"
                              controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                            {{ scope.row.name }}
                      ...
                      <el-table-column label="Title" width="150" prop="title">
                        <template slot-scope="scope">
                          <el-input tabindex="2" size="small"
                            v-if="_isallowed('write') && (scope.$index == pocCreateRow)" placeholder=""
                            style="text-align:center" v-model="scope.row.title" controls-position="right"></el-input>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.title"
                              controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                            {{ scope.row.title }}
                      ...
                      <el-table-column label="Email" width="300" prop="email">
                        <template slot-scope="scope">
                          <el-input tabindex="3" size="small"
                            v-if="_isallowed('write') && (scope.$index == pocCreateRow)" placeholder=""
                            v-model="scope.row.email" @blur="validateEmail(scope.row.email)" type="email" name="email"
                            style="text-align:center" controls-position="right"></el-input>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" type="email" @blur="validateEmail(scope.row.email)" placeholder=""
                              style="text-align:center" v-model="scope.row.email" controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                            {{ scope.row.email }}
                          </span>
                      ...
                      <el-table-column label="Work Phone #" width="175" prop="work_number">
                        <template slot-scope="scope">
                          <el-input tabindex="4" size="small"
                            v-if="_isallowed('write') && (scope.$index == pocCreateRow)" placeholder=""
                            style="text-align:center" type="text" @input="acceptNumber"
                            @blur="validatePhoneNumber(workNumberValNew)" v-model="workNumberValNew"
                            controls-position="right"></el-input>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" placeholder="" type="text" style="text-align:center"
                              @input="acceptNumber" @blur="validatePhoneNumber(workNumberVal)" v-model="workNumberVal"
                              controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                      ...
                      <el-table-column label="Mobile Phone #" width="175" prop="mobile_number">
                        <template slot-scope="scope">
                          <el-input tabindex="5" size="small"
                            v-if="_isallowed('write') && (scope.$index == pocCreateRow)" placeholder=""
                            style="text-align:center" @input="acceptNumber"
                            @blur="validateMobPhoneNumber(mobNumberValNew)" v-model="mobNumberValNew"
                            controls-position="right"></el-input>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" placeholder="" style="text-align:center" @input="acceptNumber"
                              @blur="validateMobPhoneNumber(mobNumberVal)" v-model="mobNumberVal"
                              controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                      ...
                      <el-table-column label="Notes" width="350">
                        <template slot-scope="scope">
                          <el-input tabindex="6" size="small"
                            v-if="_isallowed('write') && (scope.$index == pocCreateRow)" placeholder=""
                            style="text-align:center" v-model="scope.row.notes" controls-position="right"></el-input>
                          <span v-if="pocRowId == scope.row.id && scope.$index !== pocCreateRow">
                            <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.notes"
                              controls-position="right"></el-input>
                          </span>
                          <span v-if="pocRowId !== scope.row.id && scope.$index !== pocCreateRow">
                            {{ scope.row.notes }}
                      ...
                      <el-table-column label="Actions" width="115" v-if="_isallowed('write') || _isallowed('delete')"
                        fixed="right" align="center">
                        <template slot-scope="scope">
                          <el-button type="default" size="mini" @click="saveContractPOC(scope.$index, scope.row)"
                            v-if="(_isallowed('write')) && scope.$index == pocRowIndex" v-tooltip="`Save`"
                            class="bg-primary text-light px-2">
                            <i class="far fa-save"></i>
                          </el-button>
                          <el-button size="mini" type="default" v-tooltip="`Cancel Edit`"
                            v-if="scope.$index == pocRowIndex" @click.prevent="cancelPocEdits(scope.$index, scope.row)"
                          ...
                          <el-button size="mini" type="default" v-tooltip="`Delete`" class="bg-light px-2"
                            ...
                            @click="deleteContractPoc(scope.$index, scope.row)"><i
                          ...
                          <el-button size="mini" type="default" v-tooltip="`Cancel Edit`"
                            ...
                            @click.prevent="cancelNewPoc(scope.row)" class="bg-secondary text-light px-2">
                            <i class="fas fa-ban"></i>
                          </el-button>
                        </template>
            ...
            <el-dialog :visible.sync="contractProgramsModal" append-to-body center class="p-0 users">
              TEST
            </el-dialog>
          </div>
        </div>
      ...
      <el-tab-pane>
        <span slot="label"> <i class="fa-solid fa-user mr-1"
            :class="[pane3 ? 'bootstrap-purple-text' : 'txt-secondary']"></i>
          <i v-tooltip="`Manage POCs`" class="far fa-plus-circle mr-1"
            :class="[pane3 ? 'bootstrap-purple-text' : 'd-none']" @click="openPocModal"></i>
          POCS
        </span>
        <PortfolioContractPOC />
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label"> <i class="fa-solid fa-calendar-xmark mr-1"
            :class="[pane4 ? 'text-danger' : 'txt-secondary']"></i>
          EXPIRED CONTRACTS
        </span>
...
export default {
  ...
  methods: {
    ...
    _isallowed(salut) {
      return AuthorizationService.checkPortfolioContractPrivileges("PortfolioContracts", salut, this.$route, { settingType: 'Contracts' })
    },
    getSummaries(param) {
