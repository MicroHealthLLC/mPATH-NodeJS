    ...
    <div class="col-md-10">
      <div class="right-panel">
        ...
        <div class="my-1 pb-2 buttonWrapper container-fluid">
          <div class="row px-0">
            <div class="col">
              <el-button v-if="_isallowed('write')" @click.prevent="addContract" class="bg-success text-light mb-2">
                <i class="fas fa-plus-circle mr-1"></i> Add Existing Contract(s)
              </el-button>
            </div>
            <div class="col">
              <el-input type="search" placeholder="Search by Project Name, Customer or Contract #" aria-label="Search"
                aria-describedby="search-addon" v-model="search" data-cy="">
                <el-button slot="prepend" icon="el-icon-search"></el-button>
              </el-input>
            </div>
            <div class="col pl-0">
              <el-select class="w-100 mx-2" v-model="C_projectGroupFilter" track-by="id" value-key="id" multiple
                filterable clearable name="Project Group" placeholder="Filter Contracts By Group">
        ...
        <div v-loading="!contractsLoaded" element-loading-text="Fetching your data. Please wait..."
          ...
          <el-table v-if="tableData" :data="tableData
            ...
            <el-table-column label="Contract #" prop="contract_number_id">
              <template slot-scope="scope">
            ...
            <el-table-column prop="facility_group" sortable filterable label="Group">
              <template slot-scope="scope">
                <el-select v-model="scope.row.facility_group_id" class="w-100" v-if="rowId == scope.row.id" filterable
            ...
            " @click.prevent="saveEdits(scope.$index, scope.row)" v-tooltip="`Save`" size="small"
                ...
                <el-button type="default" size="small" v-tooltip="`Cancel Edit`" v-if="scope.$index == rowIndex
            " @click.prevent="cancelEdits(scope.$index, scope.row)" class="bg-secondary btn-sm text-light px-2">
                ...
                <el-button type="default" size="small" v-tooltip="`Go To Contract`"
                  ...
                  @click.prevent="goToContract(scope.$index, scope.row)" class="bg-success text-light btn-sm px-2">
      ...
      <el-dialog :visible.sync="openUserPrivilegesDialog" append-to-body center class="addUserRole p-0">
        ...
        <div class="container-fluid p-0">
          ...
          <div class="pl-3 mt-0 row"
            v-if="viableContractUsers && viableContractUsers.length > 0 && _isallowed('write')">
            <div class="col-5 pt-0 pl-0">
              ...
              <el-select v-model="contractRoleUsers" filterable class="w-100" clearable multiple track-by="id"
                ...
                <el-option v-for="item in viableContractUsers" :value="item" :key="item.id" :label="item.fullName">
                </el-option>
              </el-select>
            </div>
            ...
            <div class="col-2 pt-0 text-right">
              ...
              <el-button type="default" @click="saveContractUserRole()" v-if="contractRoleNames && contractRoleUsers"
          ...
          <div class="pl-3 mt-0 row"
            v-if="getRolesLoaded && contentLoaded && viableContractUsers && viableContractUsers.length <= 0">
            There are currently no program users to assign to this contract. You can either add new program users from
            portfolio or remove desired user from current role in this contract.
          </div>
          <div class="mt-4 row">
            <div class="col-12 pt-0">
              <el-table v-loading="!getRolesLoaded" element-loading-spinner="el-icon-loading"
                ...
                <el-table-column prop="user_full_name" width="200" sortable filterable label="Role">
                  <template slot-scope="scope">
                    <span
                      v-if="contractUsers.data.map(t => t.role_id == scope.row) && scope.$index !== rowIndex_1 || scope.$index == rowIndex_1 && isEditingRoles">
                    ...
                    <span v-if="changeRoleMode && scope.$index == rowIndex_1">
                      <el-select v-if="bulkChangeContractRoleNames.id" v-model="bulkChangeContractRoleNames" filterable
                        ...
                        <el-option v-for="item in getRoles.filter(t => t.type_of == 'contract')" :value="item"
                          :key="item.id" :label="item.name">
                        </el-option>
                      </el-select>
                      <el-select v-if="currentRoleName && !bulkChangeContractRoleNames.id" v-model="currentRoleName"
            ...
            programUsers.filter(t => item.user_id == t.id).map(t => t.fullName).length > 0" class="userNames">
                    ...
                    <span v-if="isEditingRoles && scope.$index == rowIndex_1">
                      <el-select v-model="assignedContractUsers"
                        :disabled="assignedContractUsers && assignedContractUsers.length <= 0" filterable
                ...
                <el-table-column width="145" fixed="right" align="center"
                  ...
                  <template slot-scope="scope" class="px-0">
                    ...
                    <el-button type="default" size="mini" @click="saveBulkChangeRole(scope.$index, scope.row)" v-if="scope.$index == rowIndex_1 && changeRoleMode && (bulkChangeContractRoleNames.id || currentRoleName.id) &&
            (scope.row !== bulkChangeContractRoleNames.id && scope.row !== currentRoleName.id)" v-tooltip="`Save`"
                      class="bg-primary px-2 btn-sm text-light mx-0">
                      <i class="far fa-save"></i>
                    </el-button>
                    <el-button size="mini" type="default" @click="saveRemoveUsers(scope.$index, scope.row)"
                    ...
                    <el-button type="default" size="mini" v-if="scope.$index !== rowIndex_1 && (_isallowed('delete'))"
                      ...
                      @click.prevent="removeAllUsers(scope.$index, scope.row)" class="bg-danger px-2 btn-sm mx-0">
                    ...
                    <el-button type="default" size="mini" v-if="changeRoleMode && scope.$index == rowIndex_1"
