<template>
  <div>
    <div :class="{ 'd-none': !_isallowed('write') }">
      <el-button @click.prevent="addProjectRole" :disabled="isEditting" class="bg-primary text-light mb-3">
    ...
    <el-table v-if="tableData && tableData.length > 0" :data="tableData" height="450" class="crudRow">
      <el-table-column fixed prop="role" label="Projects Role" width="250">
        <template slot-scope="scope">
          <span
            ...
            <el-input size="small" placeholder="Enter Role Name" style="font-style: italic; color: red"
              v-model="scope.row.name" controls-position="right">
            </el-input>
          </span>
          <span v-else>
            <span v-show="scope.row.name == 'update-project' ||
      scope.row.name == 'read-project' ||
      scope.row.name == 'contribute-project'" style="color: #dc3545; font-size: 15px">*
            </span>
            {{ scope.row.name }}
          </span>
      ...
      <el-table-column label="Analytics">
        <el-table-column prop="read" label="Read" width="75">
        ...
        <el-table-column prop="write" label="Write" width="75">
          <template slot-scope="scope">
            <!-- <span 
      ...
      v-if="scope.$index == 0">
       <span v-if="isAnalyticsWrite">
        ...
        <el-table-column prop="delete" label="Delete" width="75">
          <template slot-scope="scope">
            <!-- <span 
      ...
      <el-table-column fixed="right" label="Actions" class="text-center " width="120">
        <template slot-scope="scope">
          <el-button type="default" size="small" v-tooltip="`Save role`" v-if="(isEditting && currentRow == scope.$index) ||
      (scope.$index == 0 && isEditting && scope.row.newRow) && scope.row.name"
            @click.prevent="saveNewRole(scope.$index, scope.row)" class="bg-primary btn-sm text-light">
            <i class="far fa-save"></i>
          </el-button>

          <el-button type="default" size="small" @click.prevent="editRole(scope.$index, scope.row)" v-if="!isEditting &&
      scope.row.name !== 'update-project' &&
      scope.row.name !== 'read-project' &&
      scope.row.name !== 'contribute-project'" class="bg-light btn-sm" v-tooltip="`Edit Role`">
            <i class="fal fa-edit text-primary"></i>
          </el-button>

          <el-button size="small" type="default" @click.prevent="cancelEditRole(scope.$index, scope.row)"
            v-if="isEditting && currentRow == scope.$index" class="bg-secondary btn-sm text-light ml-1"
            v-tooltip="`Cancel Edit`">
            <i class="fas fa-ban"></i>
          </el-button>
          <el-button size="small" type="default" @click.prevent="cancelCreateRole(scope.$index, scope.row)"
            v-if="scope.$index == 0 && isEditting && scope.row.newRow" class="bg-light btn-sm"
