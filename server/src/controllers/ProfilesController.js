...
const currentProfile = async (req, res) => {
  try {
    ...
    for(var project of activeProjects){
      var facilityProjects = _.filter(allFacilityProjects, function(f) { return f.project_id == project.id }) 
      var facilityIds = compactAndUniq(_.map(facilityProjects, 'facility_id'))
      var facilities = _.filter(allFacilities, function(f) { return facilityIds.includes(f.id) }) 

      if(facilities.length > 0){
        var facilityGroupIds = compactAndUniq(_.map(facilities, 'facility_group_id'))
        var facilityGroups = _.filter(allFacilityGroups, function(f) { return facilityGroupIds.includes(f.id) }) 
    ...
    responseHash.project_groups = _.map(allFacilityGroups, function(fg){ return {id: fg.id, name: fg.name} })
    responseHash.projects = _.map(allFacilities, function(fg){ return {id: fg.id, name: fg.facility_name, facility_group_id: fg.facility_group_id} })
