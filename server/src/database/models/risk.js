...
module.exports = (sequelize, DataTypes) => {
  class Risk extends Model {
    ...
    async toJSON() {
      ...
      let all_user_ids = _.compact(_.uniq(_.map(risk_users, function(n){return n.user_id})))
      ...
      const accountableUserIds = risk_users
        .filter((ru) => ru.accountable())
        .map((ru) => ru.user_id);
      const responsibleUserIds = risk_users
        .filter((ru) => ru.responsible())
        .map((ru) => ru.user_id);
      const consultedUserIds = risk_users
        .filter((ru) => ru.consulted())
        .map((ru) => ru.user_id);
      const informedUserIds = risk_users
        .filter((ru) => ru.informed())
        .map((ru) => ru.user_id);      
